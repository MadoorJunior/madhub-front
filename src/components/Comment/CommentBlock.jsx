  ...
  const toNodes = (item, level = 0) => {
    return (
      ...
      >
        {item.children &&
          level < 6 &&
          item.children.map((node) => toNodes(node, level + 1))}
      </Comment>
    )
  }
  const comment = () => {
        ...
        <div>
          <div className={s.reply}>
            <textarea
              ...
              className={s.text}
              placeholder={
                hintInfo.username ? `reply to ${hintInfo?.username}` : ""
              }
            ></textarea>
            <ul className={s.operatorContainer}>
              ...
              <div className={s.btn}>
                <button className={s.comment} onClick={comment}>
                  comment
                </button>
                <button
                  className={s.cancel}
                ...
                >
                  {content ? "clear" : "cancel"}
                </button>
              </div>
            </ul>
          </div>
          <div>{getCommentListTree(comments).map(toNodes)}</div>
        </div>
      </div>
    </div>
  )
